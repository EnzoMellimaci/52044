// Generated from c:/Users/franco/Desktop/Enzo/Sintaxis/AnalizadorNuevo/gramatica.g4 by ANTLR 4.13.2
// jshint ignore: start
import antlr4 from 'antlr4';

// This class defines a complete listener for a parse tree produced by gramaticaParser.
export default class gramaticaListener extends antlr4.tree.ParseTreeListener {

	// Enter a parse tree produced by gramaticaParser#programa.
	enterPrograma(ctx) {
	}

	// Exit a parse tree produced by gramaticaParser#programa.
	exitPrograma(ctx) {
	}


	// Enter a parse tree produced by gramaticaParser#funcion.
	enterFuncion(ctx) {
	}

	// Exit a parse tree produced by gramaticaParser#funcion.
	exitFuncion(ctx) {
	}


	// Enter a parse tree produced by gramaticaParser#parametros.
	enterParametros(ctx) {
	}

	// Exit a parse tree produced by gramaticaParser#parametros.
	exitParametros(ctx) {
	}


	// Enter a parse tree produced by gramaticaParser#instrucciones.
	enterInstrucciones(ctx) {
	}

	// Exit a parse tree produced by gramaticaParser#instrucciones.
	exitInstrucciones(ctx) {
	}


	// Enter a parse tree produced by gramaticaParser#instruccion.
	enterInstruccion(ctx) {
	}

	// Exit a parse tree produced by gramaticaParser#instruccion.
	exitInstruccion(ctx) {
	}


	// Enter a parse tree produced by gramaticaParser#leer.
	enterLeer(ctx) {
	}

	// Exit a parse tree produced by gramaticaParser#leer.
	exitLeer(ctx) {
	}


	// Enter a parse tree produced by gramaticaParser#escribir.
	enterEscribir(ctx) {
	}

	// Exit a parse tree produced by gramaticaParser#escribir.
	exitEscribir(ctx) {
	}


	// Enter a parse tree produced by gramaticaParser#asignacion.
	enterAsignacion(ctx) {
	}

	// Exit a parse tree produced by gramaticaParser#asignacion.
	exitAsignacion(ctx) {
	}


	// Enter a parse tree produced by gramaticaParser#expresion.
	enterExpresion(ctx) {
	}

	// Exit a parse tree produced by gramaticaParser#expresion.
	exitExpresion(ctx) {
	}



}